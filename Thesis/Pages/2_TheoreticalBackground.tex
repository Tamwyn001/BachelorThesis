...
\begin{figure}[H]\centering
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            height=5cm,  % Adjust the height
            xtick={-2,2},
            xticklabels = {$-\omega_{\bm{q}}$,$\omega_{\bm{q}}$},
            xticklabel style={anchor=south}, % Place xtick labels above the axis
            ytick=\empty,
            unbounded coords=jump,
            xmin=-5, xmax=5,
            xlabel = \(\omega\),
...
\begin{figure}[H]\centering
    ...
    we get the largest possibility spectrum for the scattering. The illustration is inspired by \cite{FossheimSudbo2004}.}
\end{figure}
...
\subsection{Proximity effects}
The proximity effect is a superconductive process which occurs, when the superconductor is in contact with another metal. This section is based on the work of \cite{Mjos2019}.\\
...
$\Delta$.
As pictured in the following figure, we consider two moving electrons in N. One is a free electron, and the other one is into the Fermi-sea, i.e. at a lower energy than $\epsilon_F$.
...
Moreover, due to energy conservation, the hole must have the opposite wave vector as the electron under the sea.
This is called the Andrev reflection.

The inverse process is as well possible. A Cooper pair crosses the interface and breaks into two electrons after a certain distance. Therefore, the expectation value
$\langle c_{i\uparrow}c_{i\downarrow}\rangle$ is subject to a decay. This decay is exponential regarding the penetration depth. During this process, a hole binds
with the electrom in the Fermi-sea for the charge conservation, and mirrors information about the phase of the SC. More details can be found in \cite{Mjos2019}.\\
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        ...
        \draw[-] (0,-2.5) -- (0,2.5);
        \node[anchor=north] at (-3.5,2.5) {SC ($E=\epsilon_F - \Delta$)};
        \node[anchor=north] at (3.5,2.5) {N ($E=\epsilon_F$)};
        \draw[->, thick] (a) -- ($(a)-(1.5,0)$);
        \filldraw[color=black, fill = TamLightGreen!75 , thin] ($(a)+(0.25, 0)$) circle (0.25) node[anchor=center] {$\uparrow$};
        \filldraw[color=black, fill = TamLightGreen!75 , thin] ($(a)-(0.25, 0)$) circle (0.25) node[anchor=center] {$\downarrow$};
        \node[anchor=north] at ($(a)-(0,0.2)$) {Cooper pair};
        \draw[->, thick] (b) -- ($(b)-(1.5,0)$);
        \draw[->, thick] ($(b)+(1, 0)$) -- ($(b)+(2.5,0)$);
        \draw[thick, dotted, -] ($(b)-(-1.25, 0.7)$) to[out=180, in=180, looseness=2] ($(b)+(1.25, -0.4)$);
        \draw[thick, dotted, ->] ($(b)+(1.3, -0.4)$) -- ($(b)+(2.5, -0.4)$);


        \filldraw[color=black, fill = TamLightGreen!75 , thin] (b) circle (0.25) node[anchor=center] {$\uparrow$};
        \filldraw[color=black, fill = TamYellow , thin] ($(b)+(1, 0)$) circle (0.25) node[anchor=center] {$\uparrow$};
        \draw[->, thick] (c) -- ($(c)-(1.5,0)$);
        \filldraw[color=black, fill = TamLightGreen!75 , thin] (c) circle (0.25) node[anchor=center] {$\downarrow$};
        \draw[thick, dotted, ->] ($(c)-(2.5,0)$) to[out=180, in=0] ($(a)+(1, 0)$);
        \draw[thick, dotted, -] ($(b)-(2.5,0)$) to[out=180, in=0] ($(a)+(1, 0)$);


        \filldraw[color=black, fill = TamLightGreen!75 , thin] (d) circle (0.25) node[anchor=west, xshift=6pt] {Electron};
        \filldraw[color=black, fill = TamYellow , thin] ($(d)-(0,0.575)$) circle (0.25) node[anchor=west, xshift=6pt] {Hole};
        \draw[->, thick] (7,-2) -- (7,2);
        \node[anchor=west, xshift = 3pt] at (7,2) {$E$};
        \node[anchor=west, xshift = 3pt] at (7,0) {$\epsilon_F$};
        \node[anchor=west, xshift = 3pt] at (7,-1.5) {$\epsilon_F-\Delta$};
        \node[anchor=west, xshift = 3pt] at (7,1.5) {$\epsilon_F+\Delta$};
        \draw[thin] (6.8,1.5) -- (7.2,1.5);
        \draw[thin] (6.8,-1.5) -- (7.2,-1.5);
        \draw[thin] (6.8,0) -- (7.2,0);
    \end{tikzpicture}
    \caption{The proximity effects of a superconductor (SC) and a normal metal (N). The figure is inspired by \cite{Mjos2019}. 
    The spins are drawn inside the particles and their mometum outside. This picture
    shows the Andrev reflection and the inverse process. One electron from the Fermi-sea (below $\epsilon_F$) can bind with a free electron (above $\epsilon_F$) to form a Cooper pair.
    Due to energy and charge conservation, a hole is reflected in the Fermi-sea. The inverse process is also possible. A Cooper pair can break into two electrons after a certain penetration depth.} 
\end{figure}

A normal conductor is defined by a chemical potential term and a nearest neighbour hopping term. Any material that has a non-zero hopping term will
