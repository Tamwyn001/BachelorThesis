...
\rem{why does this commutator hold? More precise explanation needed or just bc we know that phonons are bosons?}
\[
...
\begin{figure}[H]\centering
    ...
    \begin{tikzpicture}
        ...
        On the right is a simplifed version of the potential that revealed itself to be very accurate with the experiments.}
    \end{figure}
...
The pair is not ruled anymore by the Pauli principle and can take part into the superconducting condensate. \rem{a few words on the Bose-Einstein condensate?}
...
S-wave is isotropic in the momentum space whereas the d-wave has a different countour for its part with positive phase
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \coordinate (A) at (0,0);
...
\end{align*}
These are nummericaly combuersom to compute, but taking $U>0$, $V<0$ and $W=0$ in the square lattice we obtain no attraction \rem{means $V_{\bm{k}\bm{k'}\eta=1}>0?$ or 0} in the 
$\lambda_1$-channel but atraction in the s- and d-wave channels. This tells us a lot like \cite{FossheimSudbo2004} analised it. 
First the gap is highly associated to the Fourier transform of the wavefunction of the Cooper-pairs \rem{Which variable describes their wavefunction?}    
...
$\Delta_{\bm{k}} = \Delta_0(T)g_4(\bm{k})$ is therfore a good case of study. \rem{maybe find a usecase of d wave + cite}\\
