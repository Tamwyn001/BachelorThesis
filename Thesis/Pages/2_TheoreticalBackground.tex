...
energy states of the system. This simplifies a lot the notation. The mathematical foundation of this formalism
lays in the Hilbert space and requires as well its dual space. To picture the many-particles interaction we need as well
to introduce the Fock space. The mathematical accuracy is not the goal of this work, so some derivation could be done with more rigour.\\
...
\end{equation}
It's a very generic definition that uses a single particle operator $\hat{H}_0$ and the interaction operator $\hat{H}_I$. The single 
...
For example with 2 particles we have $x_1$ and $x_2\in\mathcal{X}$ and the later part of the expression reads 
...
\[
    ...
    \end{pmatrix}.
\]
We usually describe this expression as the Slatter determinant. A determinant vanishes if two rows or columns are identical. If they are the same, we have two particles 
...
\begin{align*}
    ...
    &~~~~~~~=-\Phi^{(A_1)}.
\end{align*}
This sign difference may lead to computation errors. T solve this we must give a labelling to our states when we count them, and keep it when we 
...
What we just discussed is the so-called first quantization- or wave function formalism. Now we intend to introduce a shorter manner of 
...
The creation operator adds a particle in the concerned state and rephase it:
\[
    c_{\nu}^{\dagger} |n_{1},..,n_{\nu},..\rangle = (-1)^{\sum_{\mu<\nu}n_{\mu}} (1-n_{\nu})|n_{1},..,n_{\nu}+1,..\rangle
\]
...
We're going to stick with $[A,B]_{+}$ since it's more consistent with the commutator notation $[A,B]_{-}$ (or simply $[A,B]$).\\
...
from the state $|\beta\rangle$ to the state $|\alpha\rangle$. However, this transition can be restricted by physical lays. This is the role of the matrix element.
...
some scatterings that we can't describe. As the result the overlap must be zero.\\

With a better sentence we allow only two states to be modified. Otherwise, the single-particle
...
\end{equation}
This expression modifies four states, so that the overlap of two Slatter determinant vanishes, if the determinant differs in at least four states. We kept the hats
to be consistent with the definition of $\hat{H}_0$.\\ 

The l.h.s of the equation is the matrix element $\langle\Phi_{\alpha}|\hat{v}|\Phi_{\beta}\rangle$ of the operator
...
and notice that both lead to the same result. It is important to note that both Eq.\ref{eq:SecondquantizationDoubleParticle} and \ref{eq:Translation_FourStateOverlap}
build a scalar.\\
...
\end{align*}
In the last expression we obtain a $\langle x | x '\rangle$ which can't be normalized the $\{|x\rangle\}$-basis. Instead of a Kronecker delta $\delta_{xx'}$
...
\end{align*}
We already know that the first and second quantization are equivalent. We could now insert in the Hamiltonian the definition of the field operators defined in Eq.\ref{eq:FieldOp} and \ref{eq:FieldOpDag}.
From this obtain a wave function description. Then we can just use the first to second quantization translation at Eq.\ref{eq:Translation_TwoStateOverlap} and \ref{eq:Translation_FourStateOverlap}
to get a second quantized representation. The result is a generalization of the expression with the $a$ operators.\\


\subsection{Case of study: the interactive electron gas}
...
During these processes a photon is
usually carrying the momentum transfer of the scattering from one electron to another. We're going to use the formalism we introduced to 
find a second quantization representation of the interacting Hamiltonian.\\

The system we're studying is a cube of side length $L$ with volume $\Omega = L^3$ containing $N$ electrons. Further we consider a periodic boundary condition for an arbitrary 
...
\]
Here we see that the first under braced term is the Fourier transform of a Coulomb potential $v$ and the second term is a Kronecker delta.
...
\end{align*}
and we finaly get our second quantized interaction operator:
\[
...
\]
This describes an electron transferring a momentum $\bm{q}$ to another electron. The formula tells that we first kill both electrons as they are before the interaction.
...
The following diagram is a good illustration of this process.\\
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \begin{feynman}
...
Superconductivity can be illustrated as a phase transition of a metal when its temperature lays under a crystal temperature $T_C$. In the superconductive state the material 
become a perfect diamagnet and its resistivity vanishes. Let us apply a magnetic field on superconductive metal, we will observe some shielding currents that arise on it's surface.
 They are labelled as the Faraday lines. If we take a spherical
material, the density of these line is bigger on the equator than on the poles, but this depends on the shape of the metal. We have as well an internal magnetic field, that cancels exactly the one
...
loose abruptly their magnetization over $T_c$. Type II have a mix of ordinary and superconductive properties at intermediaries temperatures.
 In this mixed states the magnetization slowly decreases while heating the metal,
 until we can't measure any Meissner state any more over $T_C$. Above an intermediary field strength, we observe as well some vortices
 ...
 \begin{figure}[H]\centering
    ...
    \begin{figure}[H]\centering
        ...
        \end{figure}
During the break of the superconductive state more and more magnetic field 
flows inside the material. Assuming that some particles are responsible for the superconductivity, the field achieve to penetrate where we observe 
...
\end{equation}
We assume that it exists a ground state $|0\rangle$, where the system is filled up with a certain amount of electron until the Fermi-energy $\epsilon_F$ is reached. Associated with this
...
The momentum $\omega$ gives the so-called spectrum of the excitation from the unique-particle system. Please remember that we set $\hbar =1$. The imaginary part serves as a damping term and is 
...
Now that we've described the non-interacting system, let us make the model more complex by introducing the interactions.
...
The propagator maps in the complex space and gives the probability amplitude of finding a particle in the state $|\bm{k},\sigma\rangle$ at a time $t$. On the other hand 
...
$\tilde{\epsilon}_{\bm{k}} =  \epsilon_{\bm{k}} + \Sigma_R$. Due to the interactions, the particle then has a much smaller lifetime. $\Sigma_I$ is anti-proportional to 
the particle's lifetime $\tau_{\bm{k}}$. As said, $\Sigma_I$ is very small in the non-interacting case. 
...
This is it, a Fermi-liquid is a system of interacting electrons and quasi-particles. These quasi-particles are not eigenstates of the interacting Hamiltonian any more.
...
An important feature to mention, that is the key understanding of the Fermi-liquid, is the on-to-one correspondence of quantum numbers between the interacting and non-interacting electrons. 
...
This is so strong, that it almost cancels out the ``switching-on'' of the interactions. It's a good decision to perturb around
the free system. Due to the fact that we are limited in the number of ground states, thermodynamic quantities moves smoothly
when we disturbed the system.\\
...
$H_{e^-e^-}$ using some pairwise interactions in Eq.\ref{eq:FermiLiquid_Hamiltonian}. Therefore, the goal is now to find an expression for the Coulomb potential.\\
...
introduce the Yakuma potential which exponentially modulates the coulomb potential:
\begin{equation}
    V_{\lambda}(r) = \frac{1}{4\pi\epsilon_0} \frac{e^2}{r} e^{-r\lambda,} \label{eq:Yakuma}
\end{equation}
This is a solid approximation since we can easily Fourier transform it and let $\lambda\rightarrow 0$ in the result.
The yields to \cite{Cupcake}:
...
with $\bm{q}$ the momentum transfer during the interaction. We see that in the same way Coulomb potential in its real
 space form doesn't depend on the respective positions but only on the distance,
...
Let's assume we have an impotent Fermi sea, where the electrons are considered non-interactive. Adding electrons requires to place them
...
The Pauli principle doesn't rule their energies any more.\\
...
Morever, due to the Coulomb interactions, the electrons aim to put as much distance as they can between them, in a minimal amount of time. Therefore,
...
Let's now take a step aside and recall the quantum oscillator. It's eigenstates are considered as bosonic. This is achieved by showing that the ladder operators
...
\]
$M$ can't be really computed due to its complexity, we hold it as a parameter here. This pairing is much weaker than the electron-photon interaction. 
...
\begin{figure}[H]\centering
    ...
    \end{figure}
\subsection{Contraction of the effective Hamiltonian}
...
\begin{figure}[H]\centering
    ...
    scatters into the shell, then the $\bm{k}'$ electron scatters in the shell as well. This is not always the case if the wave vectors don't agree, as we can see on the left figure.
...
The transformation involves two new fermionic operator $\eta$ and $\gamma$ that therefore respect \ref{eq:Fermion1} to \ref{eq:Fermion3} and reads in a matrix form:
...
Therefore, talking about degrees of freedom the two electron-hole singlets replace the $\uparrow$, $\downarrow$ degrees of freedom, These are therefore preserved.\\
...
Further we consider fermions, so the occupation numbers of the particle
 type $\eta$ or $\gamma$ in state $\bm{k}$ are labelled $n_{\eta\bm{k}}$ and $n_{\gamma\bm{k}}$ and equals either 0 or 1. In the following we assume that $E_k$ has no $\mu$-dependence
 such that we make the $N$ operator appear.
\begin{align*}
...
\end{align*}
Now we reinsert the $\mu$-dependence in $E_{\bm{k}}$ and can therefore set $\mu$ to zero in the above expression.
...
To achieve such generalization we let the formalism be open to other kind of interactions and don't restrict it to the thin shell around the Fermi-surface.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \filldraw[draw=black,fill=TamLightGreen!30]  (A) circle (1.3);
        \filldraw[draw=black,fill=TamYellow!30] (B)++(-0.5,0) arc[x radius = 2, y radius = 1.5 , start angle=180, end angle=-180];
        \draw[draw=black] (B) arc[x radius = 1.5, y radius = 2 , start angle=180, end angle=-180] ;
        \fill[fill=TamYellow!30] (B)++(0.05,0) arc[x radius = 1.45, y radius = 1.95 , start angle=180, end angle=-180];

        \fill[draw=black, fill = green!30]  (B)++(1.5,0) circle (1.5);
        \filldraw[draw=black,fill=TamDarkBlue!30] (C)++(-1,0) arc[x radius = 2, y radius = 1 , start angle=180, end angle=-180] ;
        
        \filldraw[draw=black,fill=red!30] (C) arc[x radius = 1, y radius = 2 , start angle=180, end angle=-180] ;
        \filldraw[draw=black,,fill=green!30]  (C)++(1,0) circle (1.27);
    \end{tikzpicture}
    \caption{Different gap contours in the momentum space. The s-wave is isotropic and the d-wave has a different contour for its part with positive phase. \rem{Work in progress.}}
\end{figure}
...
Logically if we have a Cooper pair formation that doesn't follow this restriction, the whole Brillouin zone should be taken into account to compute the gap.\\
...
where $\epsilon$ is measured relative to the Fermi surface. Hopefully we don't have any sharpness around the Fermi-surface \cite{FossheimSudbo2004} p.93. 
...
in \cite{FossheimSudbo2004} p.88-89. We obtain \rem{check the exponent}
\[
