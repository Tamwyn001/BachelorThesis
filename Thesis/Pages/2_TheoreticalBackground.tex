...
$\langle c_{i\uparrow}c_{i\downarrow}\rangle$ is subject to a decay. This decay is exponential regarding the penetration depth. During this process, a hole is 
created in the Fermi-sea for the charge conservation. More details can be found in \cite{Mjos2019}.\\
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        ...
        \draw[-] (0,-2.5) -- (0,2.5);
        \node[anchor=north] at (-3.5,2.5) {SC};
        \node[anchor=north] at (3.5,2.5) {N};
        ...
        \draw[->, thick] ($(b)+(0.25, 1)$) -- ($(b)+(1.75, 1)$);
        \draw[thick, dotted, -] ($(b)-(2.25, -0.7)$) to[out=180, in=180, looseness=2] ($(b)+(-2.25, 1)$);
        \draw[thick, dotted, ->] ($(b)+(-2.2, 1)$) -- ($(b)+(-0.5, 1)$);


        \filldraw[color=black, fill = TamLightGreen!75 , thin] (b) circle (0.25) node[anchor=center] {$\uparrow$};
        \filldraw[color=black, fill = TamYellow , thin] ($(b)+(0.25, 1)$) circle (0.25) node[anchor=center] {$\uparrow$};
        ...
        \draw[->, thick] (4,-2) -- (4,-2);
        \node[anchor=west] at (4,-2) {$E$};
        \node at (4,0.5) {$\epsilon_F$};
        \node at (4,0.5) {$\epsilon_F$};
    \end{tikzpicture}
\end{figure}
