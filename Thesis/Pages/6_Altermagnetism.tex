...
$\epsilon(-\bm{k},\sigma) = \epsilon(\bm{k},\sigma)$ wich result in a even band structure. Such systems are called inversion symetric altermagnets.\\
...
\end{equation*}
involving $\bm{\sigma}=(\sigma_1, \sigma_2,\sigma_3)$ the Pauli matricies. $\bm{m}_{ij}$ is the actual spin dependent hopping term. If the connection line 
between the two sites $i$ and $j$ lays on the $\bm{e}_x$ axis we have $\bm{m}_{ij} = m(0,0,1)^T$ and $\bm{m}_{ji} = m(0,0,-1)^T$ along $\bm{e}_y$ with $m$ 
