...
\begin{figure}[H]
    ...
    magnetisation. Wheras the antiferromagnet's sublattices are liked by a translation $t$ and a space inversion $\mathcal{P}$, in the altermagnet one can linked
    each sublattice with a rotation, here labelled as $\mathcal{C}$. We observe as well an alternating sign in the energy spectra of both altermagnet's sublattices \cite{Smejkal2022}\cite{Stonebraker}.}
\end{figure}


These properties puts the altermagnet in between
...
The antiferromagnet has an spin swich in every directions. Here when choosing a point to apply our operations to, we see that 
