...
We know that the momentum index should cover the entire first Brilloin zone. This means we have $n\in \left[-N_y/2 ; -N_y/2\right]$ including $0$. \\
...
\begin{equation*}
    \begin{aligned}
    F_{xx'k} =& - V_{x,x+1} F_{x}^{x+} c^{\dagger}_{x+1,k,\downarrow}c^{\dagger}_{xk'\uparrow} + V_{x,x-1} F_{x}^{x-} c^{\dagger}_{x+1,k,\downarrow} c^{\dagger}_{xk'\uparrow} \\
        &+ V_{xx}\left( F_{x}^{y+} e^{-\im k}  - F_{x}^{y-} e^{\im k}\right)
    \end{aligned}
\end{equation*} 
...
\end{equation}
Looking at $D_{k} = \mathfrak{X}_k\mathfrak{g}_k$ we have:
\begin{center}
...
\begin{equation*}
    ...
    \frac{1}{N_y} \sum_{kk'} \langle c_{xk\uparrow} c_{x\pm a,k',\downarrow} \rangle e^{\im(k + k')y} e^{\pm\im k'_a}
\end{equation*}
...
\begin{equation*}
    F^{\pm a}_i = 
    \frac{1}{N_y}\sum_{y\in\natset{N_y}} \frac{1}{N_y} \sum_{kk'} \langle c_{xk\uparrow} c_{x+a,k',\downarrow} \rangle e^{\im(k + k')\bm{r}_i} e^{\pm\im k'_a}
\end{equation*}
which after the sumation over $i$ results as we covered earlier as $1/N_y \sum_{y} e^{\im(k + k')y} = \delta_{k,-k'}$
\begin{equation*}
    F^{\pm x}_i = 
    \frac{1}{N_y} \sum_{k} \langle c_{xk\uparrow} c_{x\pm a,k,\downarrow} \rangle e^{\mp\im k_a}
\end{equation*}
...
\begin{equation}\label{eq:transfo_F_x}
    F^{\pm a}_i =\frac{1}{N_y}\sum_{k}\sum_{nn'} u_{nxk}v_{n'xk}^{\ast} \langle \mathfrak{g}_{nk} \mathfrak{g}_{n'k}^{\dagger} \rangle e^{\mp\im k_a}
\end{equation}
Recalling the use of $a$ to abstractise the axis we get:
\begin{align}
    F^{\pm x}_i =& \frac{1}{N_y} \sum_{nk} u_{nk} v_{nk}^{\ast}\left(1 - f(E_{nk})\right)\\
    F^{\pm y}_i =& \frac{1}{N_y} \sum_{nk} u_{nk} v_{nk}^{\ast}\left(1 - f(E_{nk})\right) e^{\mp \im k_y}.
\end{align}
