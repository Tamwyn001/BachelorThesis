...
Indeed the spatial repartition of the phase of the gap seems to have the same geometry as the spin polarisation of the altermagnet \cite{Smejkal2022} \rem{figure?}.
...
For the brevety we use $f(a) = V_{ia}F_{ia} c^{\dagger}_{a\downarrow} c^{\dagger}_{i\uparrow}$. Still asuming that $V_{ij}>0$ and that the interaction is attractive, we have: \rem{!! sign check for V and in frotn oh Hv!}
\begin{align*}
...
\end{align*}
Now using $H_{\text{hop}}, H_{\mu}$ and $H_{V}$ we can write the full Hamiltonian as \rem{(add the formula for the altermagnet as well. Like Jacob sayed: We can reduce to a 2x2)}
\begin{equation}
    \begin{aligned}
