...
\begin{equation}
    \begin{aligned}
        \langle I_i^r\rangle_{\text{hop}} = t\cdot\mathcal{I}\text{m}\biggl[ \sum_{\sigma}\sum_{n\in\mathcal{N}_+} 
            ...
            +\bigl(1-f(E_n)\bigr) v_{ni\sigma}\left(v_{n,i-\delta_r}^{\ast} - v_{n,i+\delta_r,\sigma}^{\ast}\right)&\biggr].
    \end{aligned}
\end{equation}
And inside the altermagnet we add the following term using the same derivation method. The matrix element is a constant regarding
...
\begin{equation}
    \begin{aligned}
        \langle I_i^r\rangle_{\text{AM}} = \mathcal{I}\text{m}\biggl[ \sum_{\sigma\sigma'}\bigl(\bm{m}_{i,i+\delta_r}\cdot\bm{\sigma}\bigr)_{\sigma\sigma'}\sum_{n\in\mathcal{N}_+} 
            f(E_n)u_{ni\sigma}\left(u_{n,i-\delta_r,\sigma}^{\ast} - u_{n,i+\delta_r,\sigma}^{\ast}\right) &\\
            +\bigl(1-f(E_n)\bigr) v_{ni\sigma}\left(v_{n,i-\delta_r}^{\ast} - v_{n,i+\delta_r,\sigma}^{\ast}\right)&\biggr].
    \end{aligned}
\end{equation}
