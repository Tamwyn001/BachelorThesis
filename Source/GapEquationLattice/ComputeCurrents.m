    ...
    for i = 1: system.Nx * system.Ny
        ...
        target_site = system.points{i};
            ...
            for n = numel(computation.E)/2 +1 : numel(computation.E)
                ...
                % hopping_x = @(spin) (conj(uim1(spin)) * ui(spin) - conj(ui(spin)) * uim1(spin)...
                %     - conj(uip1(spin)) * ui(spin) + conj(ui(spin)) * uip1(spin)) * FermiDiarac(computation.E(n), system.T, system.mu)...
                %     + (vim1(spin) * conj(vi(spin)) - vi(spin) * conj(vim1(spin))...
                %     - vip1(spin) * conj(vi(spin)) + vi(spin) * conj(vip1(spin))) * (1 - FermiDiarac(computation.E(n), system.T, system.mu));
            
                % hopping_y = @(spin) (conj(uimN(spin)) * ui(spin) - conj(ui(spin)) * uimN(spin)...
                %     - conj(uipN(spin)) * ui(spin) + conj(ui(spin)) * uipN(spin)) * FermiDiarac(computation.E(n), system.T, system.mu)...
                %     + (vimN(spin) * conj(vi(spin)) - vi(spin) * conj(vimN(spin))...
                %     - vipN(spin) * conj(vi(spin)) + vi(spin) * conj(vipN(spin))) * (1 - FermiDiarac(computation.E(n), system.T, system.mu));
